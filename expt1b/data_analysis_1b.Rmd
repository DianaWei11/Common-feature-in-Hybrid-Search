...
two.way3 <- search %>%
  ...
  ungroup()
anova_results <- anova_test(data = two.way3, formula = rt ~ trial_struc + Error(subID / (trial_struc)),
             effect.size = "pes")
...
wide_data <- wide_data %>%
  mutate(difference = common - unique)
# common - neutral = 1.37
# common - unique = 0.36
# neutral - unique = -0.76
...
searchdf  = searchdf %>%
  mutate(cond_color = 
           ifelse(cond_target == "Target1" & cond_level == "blue", "color1",
...
one.way2 <- search %>%
  ...
  group_by(subID, cond_target, cond_color) %>%
  summarise(rt = mean(RT)) %>%
  aov_car(data = ., formula = rt ~ cond_target* cond_color + Error(subID / cond_target* cond_color))
summary(one.way2)
# finally, calculate the effect size
effectsize::eta_squared(one.way2, partial = TRUE)
...
library(emmeans)
# Perform pairwise comparisons to get t score and p value
pairwise_results <- emmeans(one.way2, pairwise ~ cond_target:cond_color)
summary(pairwise_results$contrasts, adjust = "bonferroni")
# cohen's d for RT
# Aggregate RT by subID and cond_level
wide_data <- search %>%
  ...
  group_by(subID, cond_target, cond_color) %>%
  summarise(mean_rt = mean(RT), .groups = "drop") %>%
  pivot_wider(names_from = c(cond_target, cond_color), values_from = mean_rt)  # Reshape to wide format

# Calculate differences and Cohen's d
wide_data <- wide_data %>%
  mutate(difference = Target3_neutral - Target1_neutral)
# Target1_color1 - Target3_color1 = 0.45
# Target1_color2 - Target2_color2 = 0.28
# Target2_color1 - Target3_color2 = 0.37
# Target2_neutral - Target3_neutral = 0.06
# Target2_neutral - Target1_neutral = 0.03
# Target3_neutral - Target1_neutral = 0.13
...
two.way3 <- search %>%
  filter(tasks == "search") %>%
  filter(correct == 1) %>%
  filter(cond_target != "Target1") %>%
  group_by(subID, cond_target, cond_level) %>%
  summarise(rt = mean(RT), .groups = "drop") %>%
  ungroup()
anova_results <- anova_test(data = two.way3, formula = rt ~ cond_target*cond_level + Error(subID / (cond_target*cond_level)),
             effect.size = "pes")
anova_table <- get_anova_table(anova_results, correction = "none")
# Calculate Cohen's d for all pairwise comparisons of cond_level
library(effectsize)
...
library(lme4)
model <- lmer(mean_RT ~ Expt * cond_level + (1 | subID), data = mean_RT)
...
rtbar1a = ggplot(data = rtsummary1a, aes(x = cond_level, y = transrt, fill = cond_level)) + 
  ...
  theme(axis.title.y = element_text(size = 17, face = "bold", margin = margin(r = 5))) +
  theme(strip.background = element_blank(), strip.placement = "outside", strip.switch.pad.grid = unit(0, "lines"))+
  ylab("Reaction Time in 1a (ms)")+
  labs(fill = "");rtbar1a


 rtsummary1b <- df1b %>%
   group_by(cond_level) %>%
   filter(cond_level == "common" | cond_level == "unique" | cond_level == "neutral") %>%
   filter(correct == 1) %>%
   summarise(
     n = n(),
     sd = sd(RT, na.rm = TRUE),
...
one.way2 <- df %>% 
  ...
  summarise(rt = mean(RT), .groups = "drop") %>%
  ungroup() %>%
  aov_car(data = ., formula = rt ~ trial_struc*Expt + Error(subID / trial_struc*Expt))
summary(one.way2)
# finally, calculate the effect size
effectsize::eta_squared(one.way2, partial = TRUE)
...
data = search %>% 
  filter(correct == 1)
conditions <- unique(data$cond_level)
pairwise_cohens_d <- combn(conditions, 2, function(x) {
  group1 <- data %>% filter(cond_level == x[1])
  group2 <- data %>% filter(cond_level == x[2])
  d <- cohen.d(group1$RT, group2$RT)
  return(data.frame(condition1 = x[1], condition2 = x[2], cohens_d = d$estimate))
}, simplify = FALSE)

# Convert to data frame
pairwise_cohens_d_df <- do.call(rbind, pairwise_cohens_d)
print(pairwise_cohens_d_df)
...
```{r stats for why collapse targets}
collapse = data %>%
...
# one way ANOVA checcking target collapse
# Fit the model
